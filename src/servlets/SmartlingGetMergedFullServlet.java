...
public class SmartlingGetMergedFullServlet extends HttpServlet {
    ...
    public static final Logger log = Logger.getLogger(SmartlingGetMergedFullServlet.class.getName());
...
//        this.langsParserES = new LangsParser(pathES);
    ...
    public SmartlingKeyEntry getSmartlingEntry(HttpServletRequest req, HttpServletResponse resp) {
        ...
        List<String> allowedParams = Arrays.asList("fileUri", "projectId", "locale","apikey");
        ...
        if ( projectid.isEmpty() || fileuri.isEmpty() || locale.isEmpty() || apiKey.isEmpty()) {
            log.severe("locale:" + locale + ".projectid:" + projectid + ".fileUri:" + fileuri);

            return null;
        } else

            return new SmartlingKeyEntry(projectid, locale,  fileuri,apiKey);
    ...
    public static List<SmartlingKeyEntry> getTranslationFromDb(DatastoreService datastore, String projectId, String locale, String fileUri) throws UnsupportedEncodingException {
        ...
        for (int i = 0; i < records.size(); i++) {
         ...
         //   String translatedVal = ((Text)records.get(i).getProperty("translatedVal")).getValue();
            String translatedVal = records.get(i).getProperty("translatedVal").toString();


            if (locale.equalsIgnoreCase("ja-JP"))
                log.info("Get Value ja-JP:" + translatedVal);

            //String translatedValDecoded = URLDecoder.decode(translatedVal,"UTF-8");
